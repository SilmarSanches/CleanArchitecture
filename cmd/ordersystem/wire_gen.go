...
func NewGetOrdersUseCase(db *sql.DB, eventDispatcher events.EventDispatcherInterface) *usecase.GetOrdersUseCase {
	orderRepository := database.NewOrderRepository(db)
	orderGet := event.NewOrderGet()
	getOrdersUseCase := usecase.NewGetOrdersUseCase(orderRepository, orderGet, eventDispatcher)
...
func NewGetOrdersHandler(db *sql.DB, eventDispatcher events.EventDispatcherInterface) *web.WebOrderGetHandler {
	orderRepository := database.NewOrderRepository(db)
	orderGet := event.NewOrderGet()
	webOrderGetHandler := web.NewWebOrderGetHandler(eventDispatcher, orderRepository, orderGet)
...
var setOrdersGetEvent = wire.NewSet(event.NewOrderGet, wire.Bind(new(events.EventInterface), new(*event.OrderGet)))
